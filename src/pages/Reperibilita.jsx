  ...
  return (
    <button
      ...
      className={cx(
        ...
        active ? "bg-violet-500/14 ring-1 ring-violet-500/18" : "bg-white/45 hover:bg-white/60 ring-1 ring-white/45"
      )}
    >
      {children}
    </button>
...
function febGrid35() {
  // griglia lun->dom, 5 settimane. Feb 2026 parte domenica -> start lun 26/01/2026
  const start = new Date(Date.UTC(2026, 0, 26)); // 2026-01-26
  const out = [];
  for (let i = 0; i < 35; i++) {
...
const GRID = febGrid35();
const WEEKDAYS = ["Lun", "Mar", "Mer", "Gio", "Ven", "Sab", "Dom"];
const SHIFT_NIGHT = "20-8";
const SHIFT_DAY = "8-20";
const SHIFTS = [
  { key: SHIFT_NIGHT, label: "20–8", tone: "violet" },
  { key: SHIFT_DAY, label: "8–20", tone: "cyan" },
];
...
export default function Reperibilita() {
  ...
  const setDayUi = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["pcMonth", pcKind, MONTH] });
    },
  });
  ...
  // tabs statiche AIB/ANA (come avevi)
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            ...
            <div className="w-full sm:w-[420px]">
              <div className="relative">
                ...
                <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cerca nome, numero, data…" className="pl-10 pr-4" />
              </div>
            </div>
          </div>
        </div>
      ...
      {tab === "pc" && (
        <Card title="Reperibili PC ">
          <div className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <h2 className="text-xl font-extrabold text-neutral-900 truncate">Febbraio 2026</h2>
                </div>
              </div>
            ...
            {pcQuery.error ? <div className="text-sm text-rose-700">{pcQuery.error.message}</div> : null}
                ...
                <div className="overflow-x-auto">
                  <div className="min-w-[1750px]">
                    <div className="grid grid-cols-7 gap-2">
                      {WEEKDAYS.map((w) => (
                        <div key={w} className="text-[11px] font-extrabold tracking-wide text-neutral-500 px-2 py-1">
                          {w}
                        </div>
                      ))}
                    </div>
                        ...
                        return (
                          ...
                          >
                            <div className="px-3 py-2 bg-white/45 flex items-center justify-between gap-2">
                              <div className="text-xs font-extrabold text-neutral-900">{String(dayNum).padStart(2, "0")}</div>
                              <div className="text-[11px] font-extrabold text-neutral-500">{fmtDayShort(dayIso)}</div>
                            </div>

                            <div className="p-3 space-y-2">
                              {/* TAB TURNO (mostra solo 3 slot) */}
                              <div className="flex gap-2">
                                {SHIFTS.map((sh) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      onClick={() => {
                                        if (!inFeb) return;
                                        setDayUi.mutate({ kind: pcKind, day: dayIso, active_shift: sh.key });
                                      }}
                                      className={cx(
